...
class Device42API:  # pylint: disable=too-many-public-methods
    ...
    def get_ports_wo_vlans(self) -> List[dict]:
        ...
        Returns:
            List[dict]: Dict of Interface information from DOQL query.
        """
        query = "SELECT m.netport_pk, m.port as port_name, m.description, m.up_admin, m.discovered_type, m.hwaddress, m.port_type, m.port_speed, m.mtu, m.tags, d.name as device_name FROM view_netport_v1 m JOIN view_device_v1 d on d.device_pk = m.device_fk WHERE m.port is not null GROUP BY m.netport_pk, m.port, m.description, m.up_admin, m.discovered_type, m.hwaddress, m.port_type, m.port_speed, m.mtu, m.tags, d.name"
        return self.doql_query(query=query)
